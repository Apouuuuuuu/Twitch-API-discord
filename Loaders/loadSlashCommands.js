    ...
    bot.commands.forEach(async command => {
        ...
        if(command.options?.length >= 1) {
            for(let i = 0; i < command.options.length; i++) {
                ...
                else slashcommand.addStringOption(option => option.setName(command.options[i].name).setDescription(command.options[i].description).setRequired(command.options[i].required));
